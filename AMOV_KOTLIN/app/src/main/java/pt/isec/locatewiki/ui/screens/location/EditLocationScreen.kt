...
fun EditLocationScreen(
    navController: NavHostController, locationId: String
) {
    val mainState by AppDataManager.state.collectAsState()
    val foundLocation = mainState.dbLocations.find { it.id == locationId }

    var title by remember { mutableStateOf(foundLocation!!.title) }
    var otherInfo by remember { mutableStateOf(foundLocation!!.otherInfo) }
    ...
    LazyColumn(
        ...
        item {
            TextField(
                ...
                label = { Text(stringResource(id = R.string.location_title)) },
        ...
        val geoPoint = GeoPoint(foundLocation!!.geoPoint.latitude, foundLocation.geoPoint.longitude)
            ...
            Button(
                onClick = {
                    if (title.isNotBlank() && otherInfo.isNotBlank()) {
                        AppDataManager.updateLocation(locationId, title, otherInfo)
